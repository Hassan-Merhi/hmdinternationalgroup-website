...
app.get("/sitemap.xml", async (_req, res) => {
  ...
  const paths = [
    "/", "/about", "/about/story", "/about/vision", "/companies",
    ...content.companies.map((company) => `/companies/${company.slug}`),
    "/industries", "/global-reach", "/gallery", "/what-we-do", "/contact",
  ];
...
app.post("/api/admin/login", async (req, res) => {
  ...
  if (pool) {
    const result = await pool.query<{ id: string; username: string; password_salt: string; password_hash: string; active: boolean }>(
  ...
  if (username !== configuredUsername || !safeEqual(password, configured)) return res.status(401).json({ message: "Incorrect username or password" });
  req.session.isWebsiteAdmin = true;
...
app.put("/api/admin/site-content", requireAdmin, async (req, res) => {
  ...
  if (!content || typeof content.heroTitle !== "string" || !Array.isArray(content.capabilities)) return res.status(400).json({ message: "Invalid site content" });
...
app.post("/api/admin/media", requireAdmin, async (req, res) => {
  ...
  const result = await pool.query(`INSERT INTO website_media (file_name, mime_type, file_bytes, size_bytes, alt_text, caption, category, sort_order) VALUES ($1, $2, $3, $4, $5, $6, $7, COALESCE((SELECT MAX(sort_order) + 1 FROM website_media), 0)) RETURNING id, file_name AS "fileName", mime_type AS "mimeType", size_bytes AS "sizeBytes", alt_text AS "altText", caption, category, sort_order AS "sortOrder", created_at AS "createdAt"`, [fileName, mimeType, bytes, bytes.length, altText, caption, category]);
...
app.patch("/api/admin/media/:id", requireAdmin, async (req, res) => {
  ...
  const caption = text(req.body?.caption, 600);
  const category = text(req.body?.category, 80) || "General";
  const result = await pool.query(`UPDATE website_media SET alt_text = $1, caption = $2, category = $3, updated_at = NOW() WHERE id = $4 RETURNING id, file_name AS "fileName", mime_type AS "mimeType", size_bytes AS "sizeBytes", alt_text AS "altText", caption, category, sort_order AS "sortOrder", updated_at AS "updatedAt"`, [altText, caption, category, id]);
...
app.put("/api/admin/media/:id/content", requireAdmin, async (req, res) => {
  ...
  const mimeType = text(req.body?.mimeType, 80);
  const base64 = typeof req.body?.base64 === "string" ? req.body.base64 : "";
  if (!Number.isSafeInteger(id) || id <= 0 || !mediaMimeTypes.has(mimeType) || !base64) return res.status(400).json({ message: "Invalid replacement media" });
  const bytes = Buffer.from(base64, "base64");
  if (!bytes.length || bytes.length > maxMediaBytes) return res.status(400).json({ message: "Media must be between 1 byte and 8 MB" });
  const result = await pool.query(`UPDATE website_media SET file_name = $1, mime_type = $2, file_bytes = $3, size_bytes = $4, updated_at = NOW() WHERE id = $5 RETURNING id, file_name AS "fileName", mime_type AS "mimeType", size_bytes AS "sizeBytes", alt_text AS "altText", caption, category, sort_order AS "sortOrder", updated_at AS "updatedAt"`, [fileName, mimeType, bytes, bytes.length, id]);
  if (!result.rowCount) return res.status(404).json({ message: "Media not found" });
...
app.get("/api/admin/users", requireAdmin, async (_req, res) => {
  ...
  const result = await pool.query(`SELECT id, username, display_name AS "displayName", role, active, created_at AS "createdAt" FROM website_admins ORDER BY created_at`);
...
app.post("/api/admin/users", requireAdmin, async (req, res) => {
  ...
  try {
    const result = await pool.query(`INSERT INTO website_admins (username, display_name, password_salt, password_hash) VALUES ($1, $2, $3, $4) RETURNING id, username, display_name AS "displayName", role, active, created_at AS "createdAt"`, [username, displayName, credentials.salt, credentials.hash]);
...
app.patch("/api/admin/users/:id", requireAdmin, async (req, res) => {
  ...
  const current = await pool.query<{ id: string; username: string; display_name: string; password_salt: string; password_hash: string; active: boolean }>("SELECT id, username, display_name, password_salt, password_hash, active FROM website_admins WHERE id = $1", [id]);
  ...
  const result = await pool.query(`UPDATE website_admins SET display_name = $1, active = $2, password_salt = $3, password_hash = $4, updated_at = NOW() WHERE id = $5 RETURNING id, username, display_name AS "displayName", role, active, created_at AS "createdAt"`, [displayName, active, salt, hash, id]);
...
app.get("/api/admin/audit", requireAdmin, async (_req, res) => {
  ...
  const result = await pool.query(`SELECT id, admin_username AS "adminUsername", action, details, created_at AS "createdAt" FROM website_admin_audit ORDER BY created_at DESC LIMIT 100`);
...
app.post("/api/contact", async (req, res) => {
  ...
  if (!name || !email || !country || !message) return res.status(400).json({ message: "Name, business email, country and message are required" });
